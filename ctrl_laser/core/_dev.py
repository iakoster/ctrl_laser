...
class Arduino(object):
    ...
    def read_update_date(self) -> list[int]:
        ans = self.send(self._pf["update_date"].read(response=0))
        val = ans.data.unpack()[0]
        return [val // 10000, val // 100 % 100, val % 100]

    def read_timer_frequency(self) -> int:
        return self.send(
            self._pf["timer_frequency"].read(response=0)
        ).data.unpack()[0]

    def read_regime(self) -> int:
        return self.send(self._pf["regime"].read(response=0)).data.unpack()[0]

    def read_pulse_period(self) -> int:
        return self.send(
            self._pf["pulse_period"].read(response=0)
        ).data.unpack()[0]

    def read_pulse_width(self) -> int:
        return self.send(
            self._pf["pulse_width"].read(response=0)
        ).data.unpack()[0]
    ...
    def write_regime(self, regime: int) -> None:
        self.send(self._pf["regime"].write(regime, response=0))

    def write_pulse_period(self, period: int) -> None:
        self.send(self._pf["pulse_period"].write(period, response=0))

    def write_pulse_width(self, width: int) -> None:
        self.send(self._pf["pulse_width"].write(width, response=0))
