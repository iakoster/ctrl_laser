...
#define last_update 20220312 // Дата последнего обновления


#define pin_ttl_out 4 // Номер пин программного ШИМ
#define pin_sync_in 0 // Номер прерывания (0 - D2, 1 - D3)
#define pin_led_out 13 // Контроль ШИМ с помощью LED
#define led_blink_multiplier 49 // Сколько нужно импульсов для морграния светодиода

#define timer_frequency 20000 // частота таймера, Гц (макс следование импульсов freq/2)


struct {
  uint16_t preamble = 0x4c00; // преамбула этого микроконтроллера
  uint8_t min_length = 5; // минимальная длина сообщения
  enum {RX_READ, RX_WRITE, ANSWER_READ, ANSWER_WRITE, ANSWER_ERROR} operation; // индексы операций
...
struct {
  ...
  bool synced = false; // Флаг срабатывания триггера
  enum {OFF, SINGLE, PERIODIC, CONST, SINGLE_SYNC, PERIODIC_SYNC} regime = OFF; // Режим работы лазера
  uint16_t pulse_offset = 0; // Смещение импульса
  uint16_t pulse_width = 20; // Ширина импульса
  uint16_t pulse_start = 0; // Старт импульса
  uint16_t pulse_end = 20; // Конец импульса
  uint16_t pulse_period = 399; // 400 равно частоте 50 Гц (при 20кГц таймере). Меньше на 1 т.к. есть такт между 399 и 0

} state;

volatile uint16_t timer_ticks = 0; // Счетчик прерываний таймера
volatile uint16_t sync_ticks_led = 0; // Счетчик срабатываний триггера для LED
...
void setup()
{
    ...
    // Подключаем прерывания (ПИН, функция при прерывании, момент срабатывания)
    attachInterrupt(pin_sync_in, syncTrigger, RISING);
...
void ledControl() {
    ...
    switch(sync_ticks_led) {
    case 0:
      if (state.synced) {digitalWriteFast(pin_led_out, HIGH);}
      break;

    case led_blink_multiplier:
      sync_ticks_led = 0;
      break;
