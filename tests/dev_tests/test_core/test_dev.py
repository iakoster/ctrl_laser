...
class TestArduino(unittest.TestCase):
    ...
    def test_read(self) -> None:
        ...
        for method, result in (
            (self.dev.read_firmware_version, [0, 0, 1, 0]),
            (self.dev.read_update_date, [2022, 11, 7]),
            (self.dev.read_timer_frequency, 100000),
    ...
    def test_validate_pulse_width(self) -> None:
        ...
        for width in (199, 200, 201, 500):
            self.assertEqual(199, self.dev.write_pulse_width(width))

        self.dev.write_pulse_period(100)
        self.assertEqual(99, self.dev.read_pulse_width())

    def test_validate_pulse_period(self) -> None:
        self.assertEqual(2, self.dev.write_pulse_period(1))
        self.assertEqual(1, self.dev.read_pulse_width())
